<template>
  ...
  >
    <!--Product Image -->
    <div
      class="flex justify-center w-full md:w-[80%] xl:w-1/2 mx-auto transition-all duration-300"
    >
      <NuxtImg 
    ...
    <!--Product Info -->
    <div
      class="flex flex-col gap-4 justify-start w-full md:w-[85%] xl:flex-1 mx-auto transition-all duration-300"
    >
      <!--Title + Rating -->
      <div>
      ...
      <div class="flex items-center gap-3">
        <span class="text-2xl">${{ product.price }}.00</span>
        <span class="line-through text-onMainText text-lg">${{ product.oldPrice }}.00</span>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center border border-onMainText rounded-lg overflow-hidden">
          ...
          <span
            class="px-8 py-2 bg-primary text-mainText border-r border-l border-onMainText"
            >{{ quantity }}</span
          >
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <div class="relative w-full">
            <select
              ...
              class="appearance-none bg-[#282C32] border border-onOutline rounded-lg px-3 pr-10 py-3 
                     text-md w-full cursor-pointer focus:outline-none text-mainText"
            >
              <option v-for="(p, i) in product.platforms" :key="i">{{ p }}</option>
            </select>
            <i
              class="fa-solid fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-mainText text-md pointer-events-none"
            ></i>
          </div>
      ...
      <!-- Info Badges -->
      <div
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6 border-2 bg-bgLight/70
         border-onMainText py-4 px-4 rounded-lg text-center"
      >
        <div
          v-for="(info, i) in infoBadges"
          :key="i"
          class="flex flex-col sm:flex-row items-center justify-center gap-3 py-4
          hover:bg-bgDark/60 transition-all duration-300 rounded-lg px-3 cursor-pointer"
        >
          <NuxtImg 
          densities="x1" quality="80" loading="lazy" format="webp"
            :src="info.icon"
...
const infoBadges = computed(() => [
  ...
  {
    label: 'Delivery',
    value: props.product.delivery || 'Online',
...
<style scoped>
/*Force iPad (Pro + Mini) to behave like mobile */
